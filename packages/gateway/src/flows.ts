...
function openBrowser(url: string): void {
  if (process.env.JONTRIX_NO_BROWSER === '1') return;
  try {
...
async function deviceFlow(base: string, opts: GlobalOpts, agentName?: string): Promise<number> {
  ...
  const d = body as { device_code: string; user_code: string; verify_url: string; interval: number; expires_in: number };
  ...
  while (Date.now() < deadline) {
    ...
    const b = poll.body as { status?: string; error?: string; interval?: number; access_token?: string };
    if (poll.status === 200 && b.access_token) {
      const pair = poll.body as { access_token: string; refresh_token: string; scope?: string };
...
async function pasteFlow(base: string, opts: GlobalOpts, token: string, isPat: boolean): Promise<number> {
  ...
  const entry = isPat
    ? { pat: token, kind: 'pat' as const }
    : { aat: token, kind: 'aat' as const };
  writeSecret(opts.profile, {
    ...entry,
    endpoint: base,
    last4: last4(token),
...
export async function login(opts: GlobalOpts, flags: Record<string, string | boolean>): Promise<number> {
  const base = (opts.endpoint as string) ?? readProfilesEndpoint(opts.profile);
  ...
  if (tokenFlag) {
    let token = typeof flags[tokenFlag] === 'string' ? (flags[tokenFlag] as string) : '';
...
function readProfilesEndpoint(profile: string): string {
...
export async function logout(opts: GlobalOpts): Promise<number> {
...
export async function whoami(opts: GlobalOpts, json: boolean): Promise<number> {
  const secrets = readSecrets();
  const entry = secrets[opts.profile];
  if (!entry) {
    if (json) console.log(JSON.stringify({ logged_in: false }));
    else console.log('Not logged in — run: jontrix-gateway login');
    return 2;
  }
  if (entry.kind === 'pat') {
    ...
    if (!keyringAvailable()) console.error('warning: secrets stored in a 0600 file — OS keyring unavailable');
    return 0;
  }
  ...
  try {
    ...
    console.error(`could not reach ${entry.endpoint}: ${(e as Error).message}`);
    return 3;
  }
}
