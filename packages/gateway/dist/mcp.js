...
export function serveStdio(deps) {
    let buffer = '';
    ...
    process.stdin.on('data', (chunk) => {
        ...
        while ((idx = buffer.indexOf('\n')) >= 0) {
            ...
            handleLine(line, deps).catch((e) => {
                ...
                process.stderr.write(`gateway crash: ${String(e?.message ?? e)}\n`);
                process.exit(6);
            });
        }
    });
...
async function handleLine(line, deps) {
    let req;
    try {
        req = JSON.parse(line);
    }
            ...
            case 'tools/call': {
                ...
                try {
                    ...
                    if (status === 200) {
                        ...
                        if (status === 401 && deps.onDeadToken)
                            deps.onDeadToken();
                    }
                }
                catch (e) {
...
function apiError(id, e) {
    if (e instanceof ApiError) {
        return { jsonrpc: '2.0', id, error: { code: -32000, message: e.code, data: e.message } };
    }
    return { jsonrpc: '2.0', id, error: { code: -32000, message: 'INTERNAL', data: String(e?.message ?? e) } };
}
